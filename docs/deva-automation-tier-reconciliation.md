...
This doc compares V3's per-item **Tier** (Deterministic / Det+model / Model-primary / Det+human)
...
## Pattern 1 — V3 scores the checklist item's literal scope; we score the whole SC
...
matrix uses everywhere, e.g. 1.1.1's "presence is deterministic, equivalence is agentic"). V3's
checklist item is narrower — often *just* the presence half — so V3 correctly scores it as more
...
| Item | Us (SC ceiling) | V3's (item-scoped) | V3's reasoning |
|---|---|---|---|
| 2.4.2/pptx "Unique Slide Titles" | MA | **DET** for uniqueness | "Check non-empty AND unique across the deck (set comparison). Fully deterministic." Descriptiveness is a separate optional check V3 notes but doesn't fold in. |
...
## Pattern 2 — V3's more conservative on "deterministic but consequential" fixes
...
| 2.1.1/pptx "Keyboard Accessible Navigation" | MA/MECH | **Det+human / Structural rebuild** | Shares the reading-order mechanism and risk; V3 also scores the *assessment* side stricter — "operability is behavioral," not machine-assisted — a genuine, not just scope-based, disagreement. |

This matrix's own existing text for the docx contrast fix already half-concedes V3's point ("auto-
...
V3's fix-tier is scoring the *consequence* of applying it unreviewed. Both are defensible; they're
...
it's meant to answer "would ACP ever auto-apply this without a human in the loop," V3's REAUTH is
probably right — and notably, **ACP's own actual behavior already agrees with V3's**: the docx/xlsx/
...
analysis method (V3's: code-level implementation planning; ours: WCAG-text-plus-format-properties
